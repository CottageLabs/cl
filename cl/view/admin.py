...
def dropdowns():
    dropdowns = {
        "vat": ['UK','EU','Other'],
        "projects": [i['_source']['id'] for i in cl.dao.Project.query(q="*",size=1000000).get('hits',{}).get('hits',[])],
        "internals": [i['_source']['id'] for i in cl.dao.Project.query(terms={"internal":"yes"},size=1000000).get('hits',{}).get('hits',[])],
...
def geteverything(q=False,person=False,project=False,contact=False,contractor=False,ignore=[],ignoreisolated=False,datefrom=False,dateto=False):
    ...
    if 'contacts' not in ignore:
        ...
        contacts = [i['_source'] for i in cl.dao.Contact.query(q=qryobj,size=10000).get('hits',{}).get('hits',[])]
        qryobj = {'query':{'match_all':{}}}
    ...
    if 'people' not in ignore:
        ...
        if q: qryobj['query']['bool']['must'].append({'query_string':{'query':'*' + q.lstrip('*').rstrip('*') + '*'}})
        people = [i['_source'] for i in cl.dao.Account.query(q=qryobj,size=10000).get('hits',{}).get('hits',[])]
        qryobj = {'query':{'match_all':{}}}
        for item in people:
            item["name"] = item["id"]
            item["rtype"] = "people"
    ...
    linkables = {
        ...
        "commitments": [i['_source'] for i in cl.dao.Commitment.query(q=commqryobj,size=10000).get('hits',{}).get('hits',[])],
        "financials": [i['_source'] for i in cl.dao.Financial.query(q=finqryobj,size=10000).get('hits',{}).get('hits',[])]
    }
...
def report(projectid):
    project = cl.dao.Project.pull(projectid)
...
def project(project=''):
    ...
    if request.method == 'GET':
        ...
        del proj['finance']
        p = cl.dao.Project(**proj)
        p.save()
        for item in commits:
            t = cl.dao.Commitment(**item)
            t.save()
        for item in finance:
            f = cl.dao.Finance(**item)
            f.save()
        return ""
'''
...
@blueprint.route('/<itype>', methods=['GET','POST'])
def save(itype):
    klass = getattr(cl.dao, itype[0].capitalize() + subpath[1:] )
